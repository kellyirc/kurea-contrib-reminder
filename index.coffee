module.exports = (Module) ->
	...
	backInTimeLines = [
		...
		"Yeah, I'll just, y'know, totally send you
		a message back in time. Not a problem."
		...
		"If I could time travel, I'd remind <%= target %> to
		<%= task %> <%= time %> ago. But I can't!"
		"Yeah, sure, remind <%= target %> to <%= task %> <%= time %> ago,
		because I TOTALLY can send messages back in time!"
		"This is just awfully silly of you."
		"I don't even know what to say."
		"I suppose you don't have any device that can send messages
	...
	pronounConversions =
		'\\b(myself|yourself)\\b': lowerFunc (match) ->
			if match is 'myself' then 'yourself' else 'myself'
		'\\b(yours|mine)\\b': lowerFunc (match) ->
			if match is 'yours' then 'mine' else 'yours'
		'\\b(your|my)\\b': lowerFunc (match) ->
			if match is 'your' then 'my' else 'your'
		'\\b(i|me|you)\\b': lowerFunc (match) ->
			if (match is 'i' or match is 'me') then 'you' else 'me'
		'\\b(am|are)\\b': lowerFunc (match) ->
			if match is 'am' then 'are' else 'am'
		...
		constructor: (moduleManager) ->
			...
			@addRoute 'remind :args', (origin, route) =>
				...
				try
					...
					@reply origin, "Alright, I'll remind
						#{if data.own then 'you' else data.target}
						to '#{data.task}' in #{formatTime data.time}!"
					console.log data
		...
		startReminder: (data) ->
			...
			cancelSchedule = @schedule delay, =>
				...
				text = "
					Hey #{data.target}!
					#{if data.own then 'You' else data.target}
					wanted me to remind you to '#{data.task}'!
				"
